// BUCK r14 (specialist) — a copy of _jb11d11.mjs (READ ONLY, hashed) whose
// only change is that the module under test comes from $SRC, so the BEFORE
// revision can be scored without checking the tree back out. Copied rather
// than edited because editing an instrument voids the round (S1.1).
...
const mod = await import(process.env.SRC || '../../src/art/coins.js');
