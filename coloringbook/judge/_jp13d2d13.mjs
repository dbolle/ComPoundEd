...
if (mode === 'd13' || mode === 'both') {
  ...
  const PAIRS = { obverse: 'penny-obv-3.jpg', reverse: 'penny-rev-2.png' };
  for (const [side, f] of Object.entries(PAIRS)) {
    ...
    for (const size of [26, 44, 84]) {
      ...
      console.log(`penny ${side.padEnd(8)} ${String(size).padStart(3)}px (${W} device px)`);
...
if (mode === 'd2' || mode === 'both') {
  ...
  const N = 700;
  for (const f of ['penny-rev-2.png', 'penny-rev.jpg']) {
    const rb = await refBuf(f);
