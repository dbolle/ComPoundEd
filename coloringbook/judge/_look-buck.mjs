...
const B = await import('/home/USER/compounded/.claude/worktrees/agent-a1a1ebe49a797b145/src/art/coins.js');
...
const rows = [['buck', 'reverse', 'CONTROL — must be identical'], ['buck', 'obverse', 'SUBJECT']];
const layers = [], text = [];
...
text.push(`<text x="${PAD}" y="22" font-family="monospace" font-size="15" fill="#111">BEFORE (v1.66.0)          AFTER (note portrait)</text>`);
...
await sharp(Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}"><rect width="${W}" height="${H}" fill="#fff"/>${text.join('')}</svg>`))
  .composite(layers).png().toFile('/home/USER/compounded/coloringbook/judge/_look-buck.png');
console.log('wrote _look-buck.png');
