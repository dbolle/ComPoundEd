// DIME r0 — D12. `_jp12look.mjs` (cent r0, hashed) with the subject, the
// control and the reference pair parameterised.
//
// CONTROL: the QUARTER, same side, same tier, rendered and READ FIRST. It
// shares the silver palette byte for byte, bust(), coat(), struck(), the
// field/ring pair, the specular arc AND reedGeom() (the dime and the quarter
// are the two reeded coins) and differs in the portrait and the motif.
//
// I hold four priors of other people's manufacture before looking (Q5, R6).
// The control is rendered and read first for exactly that reason.
//
...
//
//   node coloringbook/judge/_jp12look.mjs control -> _jd13-control.png
//   node coloringbook/judge/_jp12look.mjs subject -> _jd13-subject.png
//   node coloringbook/judge/_jp12look.mjs big     -> _jd13-big-obverse.png, -reverse.png
...
const CELL = 340;
const REFS = { obverse: 'dime-obv-3.jpg', reverse: 'dime-rev-2.jpg' };
const SUBJECT = 'dime', CONTROL = 'quarter';
...
async function ref(side, size) {
  ...
  const svg = mod.coinSVG(SUBJECT, size, { side });
  const W = Math.round(Number(svg.match(/width="([\d.]+)"/)[1]));
...
if (mode === 'control') {
  ...
  for (const side of ['obverse', 'reverse'])
    rows.push({ labels: TIERS.map((t) => `CONTROL quarter ${side} ${t}px`), cells: await Promise.all(TIERS.map((t) => ours(CONTROL, side, t))) });
  await sheet(rows, '_jd13-control.png', 'D12 CONTROL — the QUARTER, rendered and read BEFORE the dime. Shared: bust(), coat(), struck(), reeding, field ring, specular arc.');
} else if (mode === 'subject') {
  const rows = [];
  for (const side of ['obverse', 'reverse']) {
    rows.push({ labels: TIERS.map((t) => `dime ${side} ${t}px  OURS`), cells: await Promise.all(TIERS.map((t) => ours(SUBJECT, side, t))) });
    rows.push({ labels: TIERS.map((t) => `${REFS[side]} at the SAME device px`), cells: await Promise.all(TIERS.map((t) => ref(side, t))) });
  }
  await sheet(rows, '_jd13-subject.png', 'D12 SUBJECT — dime, ours above the photograph reduced to the same device pixel count.');
} else if (mode === 'big') {
  for (const side of ['obverse', 'reverse']) {
    const svg = mod.coinSVG(SUBJECT, 380, { side });
    await sharp(Buffer.from(svg)).flatten({ background: '#ffffff' }).resize(760, 760).png()
      .toFile(HERE + `_jd13-big-${side}.png`);
    console.log(`-> ${HERE}_jd13-big-${side}.png`);
  }
}
