...
// assertion.
//
// BEFORE is `_jw14-before-coins.js`, the snapshot taken at the head of this round
...
const BEFORE = (() => {
  const src = readFileSync('coloringbook/judge/_jw14-before-coins.js', 'utf8');
  const abs = new URL('../../src/engine/money.js', import.meta.url).pathname;
  const dir = mkdtempSync(join(tmpdir(), 'jw14i-'));
