...
const A = await import('/tmp/claude-1000/-home-USER-compounded/50ad3f5d-491c-4745-97b9-38751da704e7/scratchpad/base170/src/art/coins.js');
const B = await import('/home/USER/compounded/.claude/worktrees/agent-add962eaf6d618c02/src/art/coins.js');
...
const rows = [['dime', 'obverse', 'CONTROL — must be identical'], ['nickel', 'obverse', 'SUBJECT']];
const layers = [], text = [];
...
text.push(`<text x="${PAD}" y="22" font-family="monospace" font-size="15" fill="#111">BEFORE (v1.66.0)          AFTER (nickel back ridges)</text>`);
...
await sharp(Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}"><rect width="${W}" height="${H}" fill="#fff"/>${text.join('')}</svg>`))
  .composite(layers).png().toFile('/home/USER/compounded/coloringbook/judge/_look-nback.png');
console.log('wrote _look-nback.png');
