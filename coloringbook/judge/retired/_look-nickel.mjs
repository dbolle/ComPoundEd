...
const B = await import('/home/USER/compounded/.claude/worktrees/agent-a5c6787d5f7aa04a0/src/art/coins.js');
...
text.push(`<text x="${PAD}" y="22" font-family="monospace" font-size="15" fill="#111">BEFORE (v1.66.0)          AFTER (nickel wig strands)</text>`);
...
await sharp(Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}"><rect width="${W}" height="${H}" fill="#fff"/>${text.join('')}</svg>`))
  .composite(layers).png().toFile('/home/USER/compounded/coloringbook/judge/_look-nickel-strands.png');
console.log('wrote _look-nickel-strands.png');
