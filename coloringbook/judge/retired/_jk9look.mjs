...
const BEFORE = './_jk9-before-coins.js';
...
if (process.argv[2] === 'control') {
  ...
  await sheet(rows, 'coloringbook/judge/_jk9out/_jk9-control-quarter.png', `CONTROL — the quarter, untouched this round.  ${cap}`);
} else {
  for (const side of ['obverse', 'reverse']) {
    ...
    await sheet(rows, `coloringbook/judge/_jk9out/_jk9-buck-${side}.png`, `buck ${side}.  ${cap}`);
  }
}
