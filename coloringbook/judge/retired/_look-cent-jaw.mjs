...
const rows = [['dime', 'obverse', 'CONTROL — must be identical'], ['penny', 'obverse', 'SUBJECT']];
const layers = [], text = [];
...
text.push(`<text x="${PAD}" y="22" font-family="monospace" font-size="15" fill="#111">BEFORE (v1.66.0)          AFTER (cent mid-jaw)</text>`);
...
await sharp(Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}"><rect width="${W}" height="${H}" fill="#fff"/>${text.join('')}</svg>`))
  .composite(layers).png().toFile('/home/USER/compounded/coloringbook/judge/_look-cent-jaw.png');
console.log('wrote _look-cent-jaw.png');
